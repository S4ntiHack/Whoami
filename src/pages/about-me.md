---
layout: /src/layouts/MarkdownAbout.astro
title: "Santiago Montenegro"
description: "Seguridad ofensiva"
author: "Santiago Montenegro"
image:
  url: "/images/s4nti_image.png"
  alt: "Santiago Montenegro"
---

## Whoami

Soy **Santiago Montenegro**, apasionado por la **ciberseguridad ofensiva** y la arquitectura **Cloud**. Mi enfoque combina la administración de sistemas con la mentalidad de un atacante para construir entornos más resilientes. Actualmente, centro mis esfuerzos en la convergencia entre el análisis de vulnerabilidades y la infraestructura escalable, validando mis conocimientos mediante laboratorios prácticos y formación técnica continua.

## 🛠️ Habilidades Técnicas

### Seguridad Ofensiva (Red Team)
* **Pentesting & Auditoría:** Explotación de vulnerabilidades y análisis de seguridad con **Metasploit**, **Nmap**, **Burp Suite**, etc.
* **Wireless Security:** Auditoría de redes inalámbricas mediante **Aircrack-ng** y análisis de tráfico con **Wireshark**.
* **Offensive Scripting:** Desarrollo de herramientas personalizadas y automatización de tareas utilizando **Python** y **Bash**.

### Cloud & Infraestructura
* **Gestión de AWS:** Configuración y securización de servicios críticos: **EC2, VPC, IAM, RDS, ECS**.
* **Disponibilidad y Escalabilidad:** Implementación de arquitecturas robustas con **Load Balancers** y **Auto Scaling**.
* **Infraestructura como Código (IaC):** Automatización de despliegues mediante **AWS CloudFormation** y gestión vía **AWS CLI**.

### Sistemas y Redes
* **Administración de Sistemas:** Gestión de SO de servidor y cliente (**Linux/Windows**) con enfoque en seguridad.
* **Networking Avanzado:** Configuración y  optimización de servicios de infraestructura (**DHCP, DNS, FTP**).
* **Gestión de Datos:** Administración de bases de datos (**RDS/SQL**) asegurando la integridad, disponibilidad y seguridad de la información.
* **Servicios e Infraestructura:** Implementación de servicios de red y aplicaciones web en entornos multiplataforma y virtualizados.

## 🎓 Formación Académica
- **Grado Superior en Administración de Sistemas Informáticos en Red (ASIR) - En curso**<br>
- **Grado Medio en Sistemas Microinformáticos y Redes (SMR)***<br>

## Certificaciones y insignias 

### Hacking Ético
- [Web Penetration Tester (eWPT)](https://certs.ine.com/f8dcd964-c569-4d50-8518-5f7e9aa42707#acc.folhg8IV)
- [Hacking WIFI](https://www.udemy.com/certificate/UC-ae991d3a-9e5c-41c7-89b5-95cd7f3ec4b9/)
- [Junior Penetration Tester (eJPT)](https://certs.ine.com/29311510-a52d-4bf5-97df-a4956bdc8013#acc.tSvqu4jg)
- [Python Ofensivo](https://www.credential.net/10f3fe24-45ad-4b24-bfe5-d75c3368ba27?username=santiagooffsec11)
- [Introducción a Hacking](https://www.credential.net/f442c358-a61b-4fc8-87e8-3fbe13ceefb6?username=santiagooffsec11)
- [Introducción a Linux](https://www.credential.net/49ad29e0-3ea4-452f-8475-273e1037d646?username=santiagooffsec11)

### Cloud Computing
- [AWS Certified Cloud Practitioner (CLF-C02)](https://www.credly.com/badges/9c3aab82-a20d-4c07-869c-f88808ef6d7c/linked_in_profile)
- [AWS Cloud Web Application Builder](https://www.credly.com/badges/e05c210a-b18c-4c75-b575-6e9a33060d61/linked_in_profile)
- [AWS Cloud Quest: Generative AI Practitioner](https://www.credly.com/badges/c1147529-1f00-41e6-8ebc-65ba72e6c8ca/linked_in_profile)
- [AWS Cloud Quest: Cloud Practitioner](https://www.credly.com/badges/5c928952-1ee7-4072-ae39-040fdeb4ff37/linked_in_profile)
- [AWS Cloud Foundations](https://www.credly.com/badges/d65f80df-6461-4c15-80d1-72ecab91364f/linked_in_profile)

## Contacto
- [Email](mailto:s4nti.contact@gmail.com)
- [LinkedIn](https://www.linkedin.com/in/santiago-montenegro-395947342)
